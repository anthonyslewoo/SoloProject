...
class User:
    ...
    def get_by_id(cls, user_id):
        ...
        ## checks if id result is less than one, if so cannot find user because not in db
        if len(result) < 1:
            return False
        return cls(result[0])

    ## method puts all users into a class method function, can be used in home pages,etc
    @classmethod
    def get_all(cls):
